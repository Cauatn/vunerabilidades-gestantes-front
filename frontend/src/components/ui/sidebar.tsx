...
function readSidebarCookieOpen(fallback: boolean): boolean {
...
function SidebarProvider({
	...
	const [_open, _setOpen] = React.useState(() => readSidebarCookieOpen(defaultOpen))
...
function SidebarMenuSubButton({
	...
	return (
		<Comp
			...
			data-active={isActive}
			className={cn(
				'text-n-700 [&>svg]:text-n-700 flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 outline-hidden focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0',
				size === 'sm' && 'text-xs',
				size === 'md' && 'text-sm',
				'group-data-[collapsible=icon]:hidden',
				className,
