			...
			<div className="flex flex-1 flex-col items-center gap-10 py-10 text-center">
				...
				<div className="flex max-w-2xl flex-col gap-3">
					<p className="text-xl font-semibold text-n-900">
						Leia cada pergunta juntamente com a gestante e selecione
						a alternativa que melhor representa sua situação atual.
					</p>
					<p className="text-sm text-n-600">
						O formulário aceita somente uma resposta por pergunta. O
						resultado auxilia na tomada de decisão clínica e não
...
export function FormularioPage() {
	...
	if (!iniciado) {
		return (
			<AvisoInicial
				onInit={handleInit}
				carregando={carregandoQuestionario}
			/>
		)
	}
		...
		try {
			const { data } = await enviarAvaliacao.mutateAsync({
				...
				answers: Object.entries(respostas)
					.filter(([questionId]) =>
						perguntasVisiveis.some(
							(pergunta) => pergunta.id === questionId,
						),
					)
	...
	async function persistRecomendacoes(next: RecomendacaoGestante[]) {
		...
		return await atualizarRecomendacoes.mutateAsync({
			...
			recommendations: next.map((item, order) => ({
				...
				text: [item.titulo, item.observacoes]
					.filter(Boolean)
					.join('\n'),
				order,
			})),
						...
						{isEtapaResultado ? (
							<>
								...
								<RecomendacoesGestante
									...
									isSubmitting={
										atualizarRecomendacoes.isPending
									}
								/>
							</>
						) : (
