...
export function createGestantesColumns({
	...
	return [
		...
		},
		{
			id: 'cpf',
			header: 'CPF',
			cell: ({ row }) => row.original.identifiers.cpf ?? <span className="text-n-400">—</span>,
		},
		{
			id: 'cns',
			header: 'CNS',
			cell: ({ row }) => row.original.identifiers.cns ?? <span className="text-n-400">—</span>,
		},
		{
			id: 'vulnerabilidade',
		...
		},
		{
			id: 'actions',
			cell: ({ row }) => (
				<GestanteActionsCell gestante={row.original} onVerPerfil={onVerPerfil} onEditar={onEditar} />
			),
		},
	]
}
