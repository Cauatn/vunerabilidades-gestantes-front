...
export function useGetHealthUnits() {
	const [page, setPage] = useQueryState('unidadePagina', parseAsInteger.withDefault(1))
	const [busca, setBusca] = useQueryState('unidadeBusca', parseAsString.withDefault(''))

	const query = useQuery({
		queryKey: [...healthUnitsQueryKey, { page, busca }],
		queryFn: () => getHealthUnits({ page, pageSize: PAGE_SIZE, name: busca }),
