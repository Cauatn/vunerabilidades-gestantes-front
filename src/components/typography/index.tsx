...
function Heading({ size = 'display', as, className, ...props }: HeadingProps) {
	const Tag = (as ??
		(size === 'h2' ? 'h2' : size === 'subheading' ? 'h3' : 'h1')) as
		'h1' | 'h2' | 'h3'
	...
	return (
		<Tag
			className={cn(
				'text-n-700',
				size === 'display' &&
					'text-balance text-heading-1 font-semibold tracking-tight',
...
function Body({ className, ...props }: React.ComponentProps<'p'>) {
	return (
		<p
			className={cn('text-body font-normal text-n-700', className)}
			{...props}
		/>
	)
}
...
function Description({ className, ...props }: React.ComponentProps<'p'>) {
	return (
		<p
			className={cn(
				'text-body font-normal text-n-500 text-balance',
				className,
			)}
			{...props}
		/>
...
function Caption({ className, ...props }: React.ComponentProps<'span'>) {
	return (
		<span
			className={cn('text-caption font-normal text-n-700', className)}
			{...props}
		/>
	)
}
